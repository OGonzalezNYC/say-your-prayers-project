class Api::V1::AuthController < ApplicationController
  ...
  def create
    ...
    if @user && @user.authenticate(user_login_params[:password]) # The #authenticate method of User comes from BCrypt.
      ...
      render json: { user: UserSerializer.new(@user), jwt: token }, status: :accepted # The user data cab then be stores in the application's state (Rails, for example, I think?), while the token can be stored client-side.
    else
